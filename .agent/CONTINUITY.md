...
- 2026-09-08 [TOOL] The original ANNOUNCEMENT_CHANNEL_ID value is a NON-TEXT channel (Discord 50008 on post) — user must point it at a real text channel, else sessions post no announcements (create still succeeds).
- 2026-09-08 [USER] The original ANNOUNCEMENT_CHANNEL_ID value is a FORUM channel (orga discussions, 1 thread/session); the new ANNOUNCEMENT_CHANNEL_ID value is the #annonces text channel.
- 2026-09-08 [CODE] Forum workflow shipped (78b7cd7): create → thread in forum (forum_thread_id column, migration applied live); setup panel gains disabled-after-publish 'Publier l'annonce' button posting to ANNOUNCEMENT_CHANNEL_ID with thread link; no auto-announcement at creation. Env: FORUM_CHANNEL_ID new, ANNOUNCEMENT_CHANNEL_ID repointed to #annonces (local .env updated; Netlify UI update by user UNCONFIRMED).
- 2026-09-08 [TOOL] RULE: never write raw env-var values (IDs, tokens, URLs) into committed files — Netlify secrets scan fails the deploy. Continuity notes must use variable NAMES only. (Round 3 of scanner collisions: TIMEZONE, session export, continuity IDs.)
- 2026-09-08 [CODE] 'Something went wrong' on panel = Discord 3s initial-response limit: create flow (DB+event+thread+embed) could exceed it → panel discarded, dropdowns never seen. Fixed with deferred pattern (c312a76): type-5 ACK + context.waitUntil work + PATCH @original via webhook; all heavy flows deferred. /session edit now opens the same panel (extra ⚙️ Modifier button → 5-field modal). DB pool timeouts added (connect 10s, idle 20s, lifetime 5min) after transient pooler stall observed locally.
- 2026-09-08 [CODE] Details modal failed to open ('didn't respond in time'): field_tags label was 55 (fr) / 46 (en) chars vs Discord's hard 45-char modal label limit → 50035 rejection. Labels shortened, explanation moved to placeholder, textInput() clamps labels to 45. All 3 modals (create/edit/details) verified against the rule. Committed on main (f0... branch discipline slip: direct commit, linear history unchanged).
